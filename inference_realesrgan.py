...
def main():
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(process_image, idx, path, args) for idx, path in
                   enumerate(paths)]
        for future in as_completed(futures):
            result = future.result()
            if result:
                print(f'Image processed and saved at: {result}')

def process_image(idx, path, args):
    model_path, model, netscale = get_model(args)
    ...
    with profiler.profile(use_cuda=True, profile_memory=True) as prof:
        ...
        print(f'Processed {imgname}')
        return save_path


def get_model(args):
    custom_model = [file.split(".")[0] for file in os.listdir(WEIGHTS_DIR)]
    ...
    elif args.model_name in custom_model:  # x4 RRDBNet model
        model = RRDBNet(num_in_ch=3, num_out_ch=3, num_feat=64, num_block=23, num_grow_ch=32,
                        scale=4)
        netscale = 4
        file_url = [
        ...
        if not os.path.isfile(model_path):
            ...
            for url in file_url:
                ...
                model_path = load_file_from_url(
                    ...
                    file_name=None)
    return model_path, model, netscale
